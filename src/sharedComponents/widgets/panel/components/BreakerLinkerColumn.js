...
const BreakerLinkerColumn = React.memo(
    ({ columnIndex, edge, onBreakerLinkedClick, nodesMap }) => {
        ...
        return (
            <div className={cx('breaker-linker-column', `breaker-linker-column-${side}`)}>
                {edgesData.map((edgeData) => {
                    const isLinkedProp =
                        nodesMap[edgeData.source][isLinked] &&
                        nodesMap[edgeData.target][isLinked] &&
                        nodesMap[edgeData.target][parentBreaker];
